...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!label) {
        ...
        label.font = [UIFont fontWithName:@"HelviticaNeue-Medium" size:10];
...
- (void) addFiltersToQueue {
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        CIFilter *whiteSpecks = [CIFilter filterWithName:@"CIColorMatrix" keysAndValues:kCIInputImageKey, randomImage, @"inputRVector", [CIVector vectorWithX:0.0 Y:1.0 Z:0.0 W:0.0], @"inputGVector", [CIVector vectorWithX:0.0 Y:1.0 Z:0.0 W:0.0], @"inputBVector", [CIVector vectorWithX:0.0 Y:1.0 Z:0.0 W:0.0], @"inputAVector", [CIVector vectorWithX:0.0 Y:0.01 Z:0.0 W:0.0], @"inputBiasVector", [CIVector vectorWithX:0.0 Y:0.0 Z:0.0 W:0.0], nil];
        
        CIFilter *darkScratches = [CIFilter filterWithName:@"CIColorMatrix" keysAndValues:kCIInputImageKey, otherRandomImage, @"inputRVector", [CIVector vectorWithX:3.659f Y:0.0 Z:0.0 W:0.0],
                                   ...
                                   @"inputBiasVector", [CIVector vectorWithX:0.0 Y:1.0 Z:1.0 W:1.0], nil];
...
- (void)sendButtonPressed:(id)sender {
    ...
    if ([[UIApplication sharedApplication] canOpenURL:instagramURL]) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"No Instagram App", nil) message:NSLocalizedString(@"The Instagram app isn't installed on your device. Please install it form App Store.", nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"OK", "OK button") otherButtonTitles:nil];
        [alert show];
    }
}
...
- (void) alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (buttonIndex != alertView.cancelButtonIndex) {
        ...
        if (success) {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Could't save image", nil) message:NSLocalizedString(@"Your cropped and filtered image couldn't be saved. Make sure you have enough disk space and try again.", nil) delegate:nil cancelButtonTitle:NSLocalizedString(@"OK", @"OK button") otherButtonTitles:nil];
            [alert show];
            return;
        }
